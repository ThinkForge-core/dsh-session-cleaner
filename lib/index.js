...
export function apply(ctx) {
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        // Guard: any attached agent (running or idle) means the session's
        // lifecycle is owned by an agent loop — never tear that down blindly.
        const agent = agents?.get?.(sessionId);
        if (agent !== undefined) {
          const status = agent.status ?? "attached";
          return sendJson(res, {
            ok: false,
            error: {
              code: "refused",
              message: `session "${sessionId}" has an attached agent (${status}); stop it before deleting`,
            },
          });
        }
...
export { detachLiveStore, detachWorkspaces, removeArtifacts };
