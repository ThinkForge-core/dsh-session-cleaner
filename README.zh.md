...
客户端部分在侧边栏会话行 **⋮ 菜单底部**添加一个 **🗑 "删除此会话"** 菜单项。完全不渲染菜单的行——空白的 "新会话" 占位行（启动失败、日志从未写入的会话）——则会在**行内直接显示一个 🗑 按钮**（悬停时出现）。两个入口在侧边栏提供 `data-session-id` 属性时（`dsh-multiroot-workspace` 已提供）都优先用该属性定位会话，否则回退到按行标题匹配；标题有歧义（重名）时回退路径会被跳过以保证安全。
